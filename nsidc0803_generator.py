...
def add_nc_coordinate_values(nc_path, date, hemisphere):
    ...
    with Dataset(nc_path, 'a') as ds:
        ...
        x_vals = np.linspace(
            x_origin + x_pixel_size/2,
            x_origin + (xdim - 0.5) * x_pixel_size,
            xdim
        )
        ...
        y_vals = np.linspace(
            y_origin + y_pixel_size/2,
            y_origin + (ydim - 0.5) * y_pixel_size,
            ydim
        )
...
def encode_binary_to_nc(nc_path, binary_path, hemisphere):
    ...
    if len(grid_data) != expected_size:
        raise ValueError(f"Grid data size {len(grid_data)} doesn't match expected {expected_size}")
...
@click.command()
@click.option('--binary-dir', '-b', required=True, type=click.Path(exists=True),
...
@click.option('--hemisphere', '-h', type=click.Choice(['north', 'south', 'both']), 
              default='both', help='Hemisphere to process')
...
def main(binary_dir, output_dir, template, start_date, end_date, hemisphere, verbose):
    ...
    # Summary
    print()
    print(f"Processing complete: {total_success}/{total_attempted} files created successfully")
